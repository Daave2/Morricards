...
export default function AmazonClient() {
  ...
  const { toast } = useToast();
  const { settings, settingsLoaded, setSettings } = useApiSettings();
  ...
  const runAnalysis = useCallback(async (
    analysisParams: { imageDataUri?: string, skus?: string[] },
    ...
    if (!locationId) {
        toast({ variant: 'destructive', title: 'Store ID Missing', description: 'Please set a store ID in settings.'});
        setIsLoading(false);
        return;
    }

    toast({ title: 'Starting Analysis...', description: 'Analyzing products...' });
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        toast({
          ...
          description: 'No SKUs were found in the image or provided list.',
        });
      }
    } catch (error) {
      toast({
  ...
  const handleImageSelected = useCallback(async (file: File) => {
    ...
    await runAnalysis({ imageDataUri });
  }, [runAnalysis]);
  ...
  useEffect(() => {
    ...
    if (settingsLoaded && skusFromUrl) {
      const skuList = skusFromUrl.split(',');
      runAnalysis({ skus: skuList }, locationFromUrl);
      
      // Update settings if a different locationId was provided
      if (locationFromUrl && locationFromUrl !== settings.locationId) {
  ...
  }, [settingsLoaded, searchParams, router, runAnalysis, settings.locationId, setSettings]);
