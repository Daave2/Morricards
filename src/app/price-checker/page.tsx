              ...
              {validationLog.map(result => (
                <Card key={result.id} className={result.isCorrect ? 'bg-green-50/50' : 'bg-red-50/50 border-destructive'}>
                  <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-start">
                    <Image src={result.imageDataUri} alt="Price ticket" width={150} height={100} className="rounded-md border-2 object-cover" />
                    <div className="flex-grow space-y-3">
                      <div className="flex justify-between items-start">
                         <h3 className="font-bold text-lg">{result.product?.name || result.ocrData?.productName || 'Unknown Product'}</h3>
